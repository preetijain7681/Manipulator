...
def NextState(X_current, qdot, delta_t, qdot_lim, gripper_state, write = None):
    ...
    Inputs: 
        1. X_current: 12-array representing the current configuration of the robot (3 variables for the chassis configuration [phi, x,y], 5 variables for the arm configuration, and 4 variables for the wheel angles, the wheel sequence start from the left front wheel of Youbot and go clock wise).
...
def main():
    ...
    l = 0.47/2.0
    w = 0.3/2.0    
    X_final_x = 1.00-l-0.1
    X_init_x = 0
    qdot = np.zeros(9)
    N = T/delta_t
    # joint = np.array([np.pi/6,np.pi/6,np.pi/6,np.pi/6,np.pi/6])
    u_total_1 = np.array([10,10,10,10])
    u_total_2 = np.array([-10,10,-10,10])
